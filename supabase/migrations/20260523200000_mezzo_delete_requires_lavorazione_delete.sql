-- Blocca delete mezzo se resta qualsiasi lavorazione non soft-deleted (in corso o archiviata).
...
begin
  ...
  select count(*)::int into v_lavorazioni
  from public.lavorazioni l
  where l.mezzo_id = p_mezzo_id and l.deleted_at is null;

  if v_lavorazioni > 0 then
    raise exception 'Impossibile eliminare il mezzo: elimina le lavorazioni collegate (concludere o archiviare non basta).';
  end if;
...
comment on function public.delete_mezzo(uuid) is
  'Elimina mezzo dopo purge storico; blocca se restano lavorazioni non eliminate o preventivi per mezzo_id.';
