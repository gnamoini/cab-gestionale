...
export function CalendarV2Section() {
  ...
  // eslint-disable-next-line react-hooks/set-state-in-effect -- lint phase2: preserve existing hook contract
    if (!isLoading && !isError) setHasLoadedOnce(true);
  }, [isLoading, isError]);

  const isMonthLoading = hasLoadedOnce && live.integrityView.isFetching;
  ...
  return (
    <>
      <section aria-label="Calendario operativo" className={`${dsSurfacePanelStatic} min-w-0 max-w-full p-4 sm:p-5`}>
        <div className="mb-4 flex min-w-0 items-start justify-between gap-3 flex-nowrap sm:flex-wrap">
