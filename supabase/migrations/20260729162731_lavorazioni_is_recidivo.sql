-- is_recidivo su lavorazioni + search tokens + list RPC

ALTER TABLE public.lavorazioni
  ADD COLUMN IF NOT EXISTS is_recidivo boolean NOT NULL DEFAULT false;
...
AS $$
  SELECT public.normalize_search_text(
    concat_ws(' ',
      ...
      CASE WHEN l.repair_present THEN 'riparazione' ELSE NULL END,
      CASE WHEN l.is_recidivo THEN 'recidivo' ELSE NULL END,
      m.targa,
      m.numero_scuderia,
      m.telaio_num,
      m.marca_telaio,
