-- list_lavorazioni_paginated — keyset pagination (SECURITY INVOKER, RLS-transparent)

begin;

create index if not exists idx_lavorazioni_list_keyset
  on public.lavorazioni (created_at desc, id desc)
...
as $$
  with filtered as (
    ...
    where l.deleted_at is null
      ...
      and (
        p_search is null
        or l.note ilike '%' || p_search || '%'
        or l.codice ilike '%' || p_search || '%'
      )
      and (
...
revoke all on function public.list_lavorazioni_paginated(text, int, timestamptz, uuid, text, text) from public;
grant execute on function public.list_lavorazioni_paginated(text, int, timestamptz, uuid, text, text) to authenticated;
