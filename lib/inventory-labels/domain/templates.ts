...
function supplierBandHeightMm(altPt: number): number {

  return Math.max(fontLineHeightMm(altPt) * 2 + 0.4, 4);
 ...
 * QR sinistra a tutta altezza (margini etichetta) · testi a destra · fornitore alt in fascia bassa.
...
export function computeLabelLayout(
  ...
  const supplierBandH = supplierBandHeightMm(altPt);

  const supplierTop = labelBottomMm - supplierBandH;

  const topGroupBottomMm = supplierTop - SUPPLIER_GAP_MM;



  let qr = Math.max(6, heightMm - m * 2);

  if (qrMaxSizeMm != null) qr = Math.min(qr, qrMaxSizeMm);
...
/** Alias preset 100×36 — stesso layout orizzontale QR full-height. */

export const computeLabelLayoutFullHeightQr = computeLabelLayout;
 ...
 * Logo centrato in alto · testo marca/desc/codice centrato (no QR).
 ...
 * QR sinistra · logo + marca/desc/codice a destra — etichetta cliente (no secondari).
...
function buildTemplate(
  ...
  const elements =
    ...
    layoutMode === "horizontal-qr-full-height" || layoutMode === "horizontal-qr-left"
  ...
  return {
    ...
    qr: {
      ...
      maxSizeMm: qrMaxSizeMm,

      position: layoutMode === "vertical-stack" ? "top-center" : "top-left",
...
export const LABEL_TEMPLATE_REGISTRY: Record<string, LabelTemplateDefinition> = {
  ...
  "100x36-default": buildTemplate("100x36-default", 100, 36, {
    ...
    layoutMode: "horizontal-qr-full-height",
