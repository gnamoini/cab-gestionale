...
import { isAuthSessionEstablished } from "@/context/auth-context";
...
export function useUserThemePrefsQuery(userId: string | undefined, authStatus: string) {
...
export function useUserThemeUpsertMutation(userId: string | undefined) {
  ...
  return useServiceMutation(
    (theme: PersistedThemeMode) => {
      if (!userId) return Promise.resolve({ success: false as const, error: "Utente non autenticato." });
