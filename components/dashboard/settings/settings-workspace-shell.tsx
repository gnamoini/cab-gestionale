...
/* eslint-disable react-hooks/refs -- settings workspace wires section refs and scroll anchors in JSX */
...
export function SistemaImpostazioniWorkspace({
  ...
  const settingsMenuItems = useMemo((): PageActionItem[] => [
    ...
    isDirty,
    bulkSave.isPending,
    undoUltimaConfigurazione,
    handleCancelChanges,
    handleSaveNow,
  ]);

  const settingsPageHeader = pageMode ? (
    <div className={`${SETTINGS_PAGE_HEADER_WRAP} flex items-center justify-end gap-2 min-w-0 flex-nowrap sm:flex-wrap`}>
