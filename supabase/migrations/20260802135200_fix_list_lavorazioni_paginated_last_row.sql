-- Keyset pagination: next_cursor deve essere l'ultima riga della pagina (min tuple),
-- non la prima (max). Con DESC + cursor < tuple, cursor sulla prima riga
-- faceva re-fetchare 99 righe duplicate → dedupe client → +1 riga per click.
...
as $$
  ...
  last_row as (
    ...
    from filtered f
    order by f.created_at asc, f.id asc nulls first
    limit 1
  )
