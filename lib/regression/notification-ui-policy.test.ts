...
assert.match(tokensSrc, /dsNotificationBellIconHeader/);
assert.match(primitivesSrc, /variant === "ghost" \? "header" : "default"/);
assert.match(primitivesSrc, /dsPageHeaderIconStroke/);
...
assert.match(centerBellSrc, /layerAboveNav[\s\S]*createPortal\(drawerPanel, document\.body\)/);
