 ...
 * Bridge: cab-sync lavorazioni → inbox (create + completata).
...
export function AdminLavorazioniNotificationBridge() {
  ...
  const handleNewLavorazione = useCallback(
    ...
    [mode, pathname, push, queryClient, staffEligible, touchDedup, user?.id],
  ...
  const handleLavorazioneUpdated = useCallback(
    ...
    (lavorazioneId: string) => {
      ...
      const resolveAndNotify = () => {
        ...
        const wrapped = wrapLavorazioneCompletataNotification(intent);
