...
export { LAVORAZIONI_ATTIVE_LIGHT_FILTERS, LAVORAZIONI_CHIUSE_COUNT_FILTERS, LAVORAZIONI_REPORT_FILTERS };
...
export const getLavorazioniArchivioCountServer = cache(async () => {
  const allowed = await verifyServerPageRead("lavorazioni");
  if (!allowed) return err("Permesso richiesto.");
  const sb = await createSupabaseServerUserClient();
  return fetchLavorazioniListCountRows(sb, LAVORAZIONI_CHIUSE_COUNT_FILTERS);
});
