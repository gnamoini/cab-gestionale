  ...
  return (
    <>
      ...
      <IconActionButton
        ...
        onClick={() => openPdfArtifactFromUserClick("preventivo", { id: p.id, autore })}
      >
...
export function PreventiviView({ listSurface: serverListSurface, listTier = "xl" }: GestionaleListPageProps) {
  ...
  const handleDdtAction = useCallback(
    async (preventivo: PreventivoRecord) => {
      ...
      if (existing) {
        openPdfArtifactFromUserClick("ddt", { id: existing.id });
        return;
      }
      if (!canWritePreventivi) return;

      const deferredResult = openDeferredPopup({ context: "pdf", label: "PDF DDT" });
