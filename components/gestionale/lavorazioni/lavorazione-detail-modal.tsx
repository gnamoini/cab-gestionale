...
import { dsAccentToggleOn, dsScrollbar, dsTable, dsTableRow, dsTableWrap } from "@/lib/ui/design-system";
...
export function LavorazioneDetailModal({ lavorazioneId, onClose }: { lavorazioneId: string; onClose: () => void }) {
  ...
  const tabBtn = (id: TabId, label: string) => {
    ...
    return (
      <button
        ...
        key={id}
        onClick={() => setTab(id)}
        className={`shrink-0 rounded-lg border px-2.5 py-1.5 text-[11px] font-semibold transition-colors ${on ? dsAccentToggleOn : "border-zinc-200 bg-zinc-50 text-zinc-600 hover:border-zinc-300 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-300"} ${erpFocus}`}
      >
        {label}
      </button>
    );
  ...
  return (
    ...
    >
        ...
        <div className="flex shrink-0 gap-1.5 border-b border-zinc-100 bg-zinc-50/50 px-3 py-2 dark:border-zinc-800 dark:bg-zinc-900/80 min-w-0 flex-nowrap sm:flex-wrap">
              ...
              <ul className="space-y-2">
                {preventiviUi.length === 0 ? (
                  ...
                  pagedPv.map((p) => (
                    <li
                      key={p.id}
                      className="flex items-center justify-between gap-2 rounded-lg border border-zinc-100 bg-zinc-50/60 px-3 py-2 dark:border-zinc-800 dark:bg-zinc-800/40 min-w-0 flex-nowrap sm:flex-wrap"
                    >
                      ...
                      <div className="flex shrink-0 gap-1 min-w-0 flex-nowrap sm:flex-wrap">
                        <button
                          type="button"
