...
import { AuthStandaloneCardHeader } from "@/components/gestionale/auth-standalone-page";
import { AUTH_STANDALONE_LOGO_SUBTITLE } from "@/components/gestionale/cab-logo";
import {
  TERMS_AND_CONDITIONS_LAST_UPDATED,
  termsAndConditionsIntro,
  termsAndConditionsSections,
} from "@/lib/legal/terms-and-conditions-content";
...
function TermsAndConditionsToc({ className = "" }: { className?: string }) {
  return (
...
/** Contenuto statico termini — RSC, fuori dal client bundle. */
export function TermsAndConditionsBody() {
  return (
    <article className={termsCardClass}>
      <AuthStandaloneCardHeader
        srOnlyTitle="Termini e condizioni"
        productLabel={AUTH_STANDALONE_LOGO_SUBTITLE}
      />

      <header className="mb-6 border-b border-[color:var(--cab-border)] pb-5 text-center">
        <h1 className={dsTypoPageTitle}>Termini e condizioni</h1>
        <p className={`${dsTypoCaption} mt-2`}>
          <strong>Ultimo aggiornamento:</strong> {TERMS_AND_CONDITIONS_LAST_UPDATED}
        </p>
      </header>

      <div className="mb-6 space-y-4 border-b border-[color:var(--cab-border)] pb-6">{termsAndConditionsIntro}</div>

      <div className="lg:hidden">
        <TermsAndConditionsToc className="mb-6 rounded-[var(--ds-radius-lg)] border border-[color:var(--cab-border)] bg-[color:color-mix(in_srgb,var(--cab-surface-2)_45%,var(--cab-card))] p-4" />
      </div>

      <div className="lg:grid lg:grid-cols-[minmax(12rem,16rem)_1fr] lg:gap-10 xl:grid-cols-[minmax(14rem,18rem)_1fr]">
        <TermsAndConditionsToc className="hidden lg:sticky lg:top-24 lg:block lg:self-start" />
