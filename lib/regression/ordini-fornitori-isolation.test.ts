...
const pageView = fs.readFileSync(path.join(ROOT, "components/ordini-fornitori/ordini-fornitori-page-view.tsx"), "utf8");
...
assert.ok(fs.existsSync(path.join(ROOT, "supabase/migrations/20261031120000_ordini_fornitori_module.sql")));
assert.ok(fs.existsSync(path.join(ROOT, "supabase/migrations/20261031120100_user_permissions_ordini_fornitori.sql")));

assert.match(pageView, /OrdiniFornitoriView/);
assert.match(pageView, /GestionaleSectionGate module="ordini_fornitori"/);
assert.doesNotMatch(preventiviView, /OrdiniFornitoriView/);
