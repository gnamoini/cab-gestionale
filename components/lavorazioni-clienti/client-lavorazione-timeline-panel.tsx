...
export function ClientLavorazioneTimelinePanel({
  ...
  const ultimaModificaLabel = useMemo(() => {
    const resolveUserId = buildLavorazioneRowProfileResolver(row, user?.id ?? null, authorName);
    const info = resolveLavorazioneUltimaModifica(row, bundle, { resolveUserId });
    const when = fmtClientUpdatedAt(info.iso);
    const autore = info.autore.trim();
    if (!when || when === "—") return "—";
    if (!autore || autore === "—") return when;
    return `${when} · ${autore}`;
  }, [row, bundle, user?.id, authorName]);
