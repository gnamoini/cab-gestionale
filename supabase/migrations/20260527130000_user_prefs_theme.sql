-- Preferenze utente in app_settings (module user_prefs, key = auth.uid()).
-- Tema: value.theme = "light" | "dark"
...
begin
  ...
  -- Preferenze personali: policy dedicate per riga (key = uid).
...
drop policy if exists cap_user_prefs_select_own on public.app_settings;
create policy cap_user_prefs_select_own
on public.app_settings for select to authenticated
using (module = 'user_prefs' and key = (auth.uid())::text);

drop policy if exists cap_user_prefs_insert_own on public.app_settings;
create policy cap_user_prefs_insert_own
on public.app_settings for insert to authenticated
with check (
  module = 'user_prefs'
  and key = (auth.uid())::text
  and updated_by = auth.uid()
);

drop policy if exists cap_user_prefs_update_own on public.app_settings;
create policy cap_user_prefs_update_own
...
comment on table public.app_settings is 'Configurazione gestionale (JSON per modulo/chiave). Modulo user_prefs: preferenze personali (key = user id).';

-- bulk_upsert: blocca scrittura preferenze altrui (il tema usa upsert singolo + RLS)
...
begin
  ...
  loop
    ...
    if v_module = 'user_prefs' and v_key <> v_uid::text then
      raise exception 'Permesso negato';
    end if;
