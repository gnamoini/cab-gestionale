...
assert.match(multiSelect, /handleAddToListCommit = useCallback\(\s*\(v: string\) => addValue\(v\)/);
assert.doesNotMatch(multiSelect, /void addValue\(v\)/);
assert.match(globalSelect, /typeof result === "string" \|\| result === null/);
