...
export async function POST(_request: Request, context: RouteContext) {
  ...
  try {
    body = (await _request.json()) as { applicationId?: string };
  } catch {
  ...
  try {
    const result = await resumeFailedCaptureApply({
      captureId: id,
    ...
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "Resume fallito", code: "APPLY_FAILED" },
      { status: 400 },
    );
  }
}
